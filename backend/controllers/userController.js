...
const registerUser = asyncHandler(async (req, res) => {
  ...
  //check if iser email alredy exists
...
const logout = asyncHandler(async (req, res) => {
  res.cookie("token", "", {
    path: "/",
    httpOnly: true,
    expires: new Date(0), //Expire Cokies
    sameSite: "none",
    secure: true,
  });
...
const changePassword = asyncHandler(async (req, res) => {
  ...
  if (!user) {
    res.status(404);
    throw new Error("User not Found,please password");
  }
  //validate
  ...
  //check if old password matches in the DB
